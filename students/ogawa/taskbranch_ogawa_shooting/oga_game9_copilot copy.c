...
// プレーヤー
Player player = { COL / 2 + 1, LINE - 1, 0, 0, 'M' };    // 最下部中央
...
// ミサイルセット
void move_missile( void ) {
  if ( flg_shoot == 1 && missile.shoot == 0 ) {                 // 飛行中でない時に発射指示があったら
...
void* key_hit( void* p ) {
  while( flg_end == 0 )  {          // flg_endが0の間ループ
    if ( kbhit() ) {
      ...
      if ( last_move != '\0' ) {
        player.move_left = ( last_move == 'a' );
        player.move_right = ( last_move == 'd' );
      }
      if ( shoot ) {
        flg_shoot = 1;              // 入力列中のスペースは1回の発射指示
      }
    }
    usleep( 100000 );
  }
