...
void draw_enemy( Enemy* enemy_data );
int enemy_hit_by_missile( Enemy* enemy_data );
...
void init_enemy( void ) {
  ...
  for ( int i = 0; i < ENEMY_COUNT; i++ ) {
    ...
    enemy[ i ].col = rand() % ( COL - enemy[ i ].width + 1 ) + 1;
    enemy[ i ].line = rand() % ( LINE - enemy[ i ].height );
...
// 敵ブロックを画面に描画
void draw_enemy( Enemy* enemy_data ) {
  for ( int r = 0; r < enemy_data->height; r++ ) {
    for ( int c = 0; c < enemy_data->width; c++ ) {
      int row = enemy_data->line + r;
      int col = enemy_data->col + c;
      if ( row >= 0 && row < LINE && col >= 1 && col <= COL ) {
        gamen[ row ][ col ] = enemy_data->symbol;
      }
    }
  }
  return;
...
int enemy_hit_by_missile( Enemy* enemy_data ) {
  ...
  for ( int r = 0; r < enemy_data->height; r++ ) {
    for ( int c = 0; c < enemy_data->width; c++ ) {
      if ( ( enemy_data->line + r ) == missile.line &&
           ( enemy_data->col + c ) == missile.col ) {
        return 1;
      }
    }
  }
...
void move_enemy2( int i ) {
  if ( enemy[ i ].direction == 1 ) {               // 右方向に移動中
    if ( enemy[ i ].col + enemy[ i ].width - 1 >= COL ) { // 右端なら
      if ( enemy[ i ].line + enemy[ i ].height >= LINE ) { // 最下部ならゲームセット、プレーヤー負け
        flg_end = 2;
      } else {
        enemy[ i ].line++;                        // 一段下へ
...
void judge_hit( void ) {
  for ( int i = 0; i < ENEMY_COUNT; i++ ) {
    if ( ( enemy[ i ].hit == 0 ) && enemy_hit_by_missile( &enemy[ i ] ) ) {
